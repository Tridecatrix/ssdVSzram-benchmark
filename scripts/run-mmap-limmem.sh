...
# nohup ./scripts/run-mmap-limmem.sh > data/log.txt 2>&1 &
#
# run while logging the output and error to file both locally and to remote ssh
# nohup ./scripts/run-mmap-limmem.sh | tee data/log.txt | ssh ctoo 'cat /dev/stdin > fioLog.txt' &
...
echo "Cgrouped memory limits: ${memlims[@]}" | tee -a $RESULTSDIR/fio-config.txt
echo "Options for DIRECT: ${directopts[@]}" | tee -a $RESULTSDIR/fio-config.txt
    ...
    for rw in "${rws[@]}"; do
      for memlim in "${memlims[@]}"; do
        for directopt in "${directopts[@]}"; do
          ...
          SUBDIR="$RESULTSDIR/request-size-$bs/nproc-$nproc/$rw/memlim-$memlim/direct-$directopt"
