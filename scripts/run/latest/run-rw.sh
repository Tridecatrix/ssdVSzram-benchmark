...
# nohup ./scripts/run/latest/run-rw.sh <config_file.sh> <devicemap.sh> > data-raven3/log.txt 2>&1 &
#
...
# stdbuf -oL nohup ./scripts/run/latest/run-rw.sh $CONF $DMAP | tee log.txt | ssh ctoo 'cat /dev/stdin > fioLog'"$(hostname)"'.txt' & disown
#
#
# mom i swear this is the last time i'll duplicate a script in order to add functionality to avoid having to care about backwards compatibility please i swear i won't do it again james told me to i swear it wasn't my idea no i don't know who james is please mom don't take away my fortnite robux please no please i can't survive phd without my fortnite robux mom please i beg you please-
...
echo "RW mixes: ${rwmixes[@]}" | tee -a $RESULTSDIR/fio-config.txt
...
for compress_pct in "${compress_percentages[@]}"; do
  ...
  for bs in "${block_sizes[@]}"; do
    for nproc in "${nprocs[@]}"; do
      for rw in "${rws[@]}"; do
        for rwmix in "${rwmixes[@]}"; do 
          echo "running fio with block size $bs, $nproc processes, $rw for read/write setting, $rwmix% writes and $compress_pct% compression"

          SUBDIR="$RESULTSDIR/$rw/rwmixwrites-$rwmix/nproc-$nproc/request-size-$bs/compress-$compress_pct"
          ...
          for ioengine in "${async_ioengines[@]}"; do
            for iodepth in "${iodepths[@]}"; do
              for di in "${!dev_names[@]}"; do
                ...
                SIZE_PER_PROC="$(($totalSize/$nproc))" BS="$bs" DIR="${dev_paths[$di]}" NPROC="$nproc" RW="$rw" IOENGINE="$ioengine" IODEPTH="$iodepth" NUMA="$numa" CMPR_PNT="$compress_pct" RWMIX="$rwmix" fio $async_config --output="$SUBSUBDIR/fio_out.txt" --output-format=$outputFormat $testrunopt
          ...
          for ioengine in "${sync_ioengines[@]}"; do
            for di in "${!dev_names[@]}"; do
              ...
              echo "`date +%F/%H:%M:%S:`: beginning run"
              $HOMEdir/system_util/start_statistics.sh -d $SUBSUBDIR
              SIZE_PER_PROC="$(($totalSize/$nproc))" BS="$bs" DIR="${dev_paths[$di]}" NPROC="$nproc" RW="$rw" IOENGINE="$ioengine" NUMA="$numa" CMPR_PNT="$compress_pct" RWMIX="$rwmix" fio $sync_config --output="$SUBSUBDIR/fio_out.txt" --output-format=$outputFormat $testrunopt
              $HOMEdir/system_util/stop_statistics.sh -d $SUBSUBDIR
