...
# nohup ./scripts/run/specific/run-dump-raven4.sh > data/log.txt 2>&1 &
#
# run while logging the output and error to file both locally and to remote ssh
# stdbuf -oL nohup ./scripts/run/specific/run-dump-raven4.sh | tee data-raven4/log.txt | ssh ctoo 'cat /dev/stdin > fioLogDumpRaven4.txt' & disown
...
EXPNAME=fourth-run-dumps-raven4
